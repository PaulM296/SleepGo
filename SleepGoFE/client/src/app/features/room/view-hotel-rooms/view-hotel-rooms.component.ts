...
@Component({
  ...
  styleUrl: './view-hotel-rooms.component.scss'
})
export class ViewHotelRoomsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getUserById(userId).subscribe({
        next: (user) => {
            ...
            this.fetchRooms(RoomType.Single, paginationRequest);
        }
    });
    ...
    this.filterForm.get('roomType')?.valueChanges.subscribe(value => {
        ...
        this.fetchRooms(value, paginationRequest);
    });
}

  fetchRooms(roomType: RoomType, paginationRequest: PaginationRequest): void {
    if (!this.hotelId) {
    ...
    this.roomService.getRoomsFromHotelByRoomType(this.hotelId, roomType, paginationRequest).subscribe({
        next: (response) => {
  ...
  onPageChange(event: PageEvent): void {
    ...
    this.fetchRooms(this.filterForm.value.roomType, paginationRequest);
  }
  ...
  updateRoom(roomId: string, updatedRoom:UpdateRoomModel): void {
    this.fetchRooms(this.filterForm.value.roomType, {
      pageIndex: this.pageIndex + 1,
      pageSize: this.pageSize
    });
  }
